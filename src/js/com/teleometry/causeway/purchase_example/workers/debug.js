...
(function debugModule(global) {
    ...
    */
   System.getCWStack = function uselessGetCWStack(err) { return void 0; };
   ...
   if ('captureStackTrace' in OriginalError) {
     (function() {
       ...
       OriginalError.prepareStackTrace = function(err, sst) {
         if (ssts === void 0) {
           ...
           console.error('Error while initializing debug module', err);
         } else {
           ssts.set(err, sst);
         }
     ...
     (function() {
       function getCWStack(err) {
         ...
         if (/^\w*Error:/.test(lines[0])) {  // What platform is this seen on?
           lines = lines.slice(1);
         }
   ...
   System.stackString = stackString;
   ...
   function getStack(err) {
     ...
     var cwStack = System.getCWStack(err);
     var result;
     if (cwStack) {
       result = System.stackString(cwStack);
     } else {
